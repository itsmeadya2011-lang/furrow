...
class LLMClient:
    ...
    @retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=4, max=10), retry=retry_if_exception_type((anthropic.APIError, openai.APIError, httpx.RequestError)))
    async def complete(self, prompt: str, system: str = "", model: str | None = None) -> str:
        model = model or self.settings.model
        logger.info("llm.request", provider=self.settings.provider, model=model)
        ...
        logger.info("llm.response", length=len(result))
        return result
