...
class LLMClient:
    ...
    def ollama(self) -> AsyncOpenAI:
        if self._ollama is None:
            base_url = self.settings.ollama_base_url or os.getenv("OLLAMA_BASE_URL") or "http://localhost:11434"
            self._ollama = AsyncOpenAI(base_url=f"{base_url.rstrip('/')}/v1", api_key="ollama")
        return self._ollama
