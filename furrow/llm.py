...
class LLMClient:
    ...
    def list_files(
        self,
        directory: str | Path,
        ignore: tuple[str, ...] = (".git", "__pycache__", "node_modules", ".venv", "venv", ".mypy_cache", ".pytest_cache", "dist", "build", ".ruff_cache"),
    ) -> list[str]:
        p = Path(directory)
        if not p.exists():
            return []
        return [
            str(f.relative_to(p))
            for f in p.rglob("*")
            if f.is_file() and not any(segment in ignore for segment in f.parts)
        ]
