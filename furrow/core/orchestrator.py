...
class Orchestrator:
    def __init__(self, goal: str, client: LLMClient | None = None, output_console: Console | None = None) -> None:
        self.goal = goal
    ...
    async def run(self) -> None:
        ...
        while True:
            ...
            if self.client.settings.max_cycles > 0 and self.cycles > self.client.settings.max_cycles:
                self.console.print(f"[bold yellow]Max cycles ({self.client.settings.max_cycles}) reached. Halting.[/bold yellow]")
                break
